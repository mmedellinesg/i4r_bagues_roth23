...
use "$data/final datasets/surveydata_merged.dta", clear
  ...
  gen mis_employmentstatus = employmentstatus==.
  replace employmentstatus = 0 if employmentstatus==.
 
global endcontrols   lninc inlaborforce same_region_residence_17 i.employmentstatus mis_employmentstatus
...
***OLS
 ...
 xi:   qui areg `choice' other_region_17_mili same_region_residence_17 $controls $endcontrols, cl(cluster) a(cluster)
 ...
 xi:   areg `choice' other_region_17_mili_HET3 other_region_17_mili nationalisticregion2  $controls $endcontrols, cl(cluster) a(cluster)
    sigstar other_region_17_mili, prec(3)
...
test 	other_region_17_mili + other_region_17_mili_HET3 = 0
 ...
 xi:   areg `choice' other_region_17_mili_HET4 other_region_17_mili predictedregionalism  $controls $endcontrols, cl(cluster)  a(cluster)
    sigstar other_region_17_mili, prec(3)
...
esttab * using "$output/identity_endcontrolscombined.tex", replace cells(none) booktabs nonotes nomtitles /*nonum*/ compress alignment(c) nogap noobs nobaselevels label stats(`rowstats', labels(`rowlabels')) ///
