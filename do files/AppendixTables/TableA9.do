...
use "$data/final datasets/surveydata_merged.dta", clear
 ...
 gen groupishnessindex = (z_universalism + z_sentiment_index + z_trustindex + z_similarity)/4
...
loc experiments "z_universalism z_sentiment_index z_trustindex z_similarity groupishnessindex"

corr z_universalism sentiment_index trustindex z_similarity groupishnessindex
...
***OLS
 ...
 xi:   qui areg `choice' other_region_17_mili i.region_military $controls, cl(cluster) a(cluster)
 ...
 xi:   areg `choice' other_region_17_mili_HET3 other_region_17_mili nationalisticregion2 i.region_military $controls, cl(cluster) a(cluster)
    sigstar other_region_17_mili, prec(3)
...
test 	other_region_17_mili + other_region_17_mili_HET3 = 0
 ...
 xi:   areg `choice' other_region_17_mili_HET4 other_region_17_mili predictedregionalism i.region_military $controls, cl(cluster)  a(cluster)
    sigstar other_region_17_mili, prec(3)
...
loc rowlabels " "{\bf Panel A: Main}" "Other region" " " " " "{\bf Panel B: Binary}" "Other region (a)" " " " " "Other region $\times$ (b)" "Peripheral Nationalism" " " "P-value (a+b)" " " "{\bf Panel C: Continuous}" "Other region (a)" " " " " "Other region $\times$ (b)" "Low identification with Spain" " " "Observations" "Year Lottery FE $\times$ Province FE"  "Controls" "Region of Service FE" "  
loc rowstats ""
...
esttab * using "$output/groupishness_regionofserviceFE.tex", replace cells(none) booktabs nonotes nomtitles /*nonum*/ compress alignment(c) nogap noobs nobaselevels label stats(`rowstats', labels(`rowlabels')) ///
  mgroups("\shortstack{Universa-\\lism}" "\shortstack{sentiment\\Other\\Region}" "\shortstack{Trust\\other\\Region}" "\shortstack{Similarity\\other\\Region}" "\shortstack{Index\\Group\\Loyalty}", pattern(1 1 1 1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}))

	
	*belonging_locality belonging_province belonging_region belonging_spain belonging_EU belonging_world
