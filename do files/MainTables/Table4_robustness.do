...
label variable z_identity "Sum of national sentiment, proud to be Spanish, and flag sentiment variables"

local experiments " z_nation_sentiment z_proud_spanish z_sentiment_flag z_identity"
...
foreach choice in `experiments' {
	...
	esttab * using "$output/table_4_`choice'_`test'.tex", replace cells(none) booktabs nofloat nonotes nomtitles compress alignment(c) nogap noobs nobaselevels label stats(`rowstats', labels(`rowlabels')) title("`test'"\label{tab4_`choice'_`test'}) substitute(\_ _) ///
